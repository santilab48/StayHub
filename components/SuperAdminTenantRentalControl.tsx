...
export default function SuperAdminTenantRentalControl({tenantId}:{tenantId:string}){
  ...
  const load=async()=>{
    ...
    const rows=(r||[]) as Req[];setRequests(rows)
    const urls:Record<string,string>={}
    for(const item of rows){if(!item.slip_path)continue;const {data}=await supabase.storage.from('stayhub-rental-slips').createSignedUrl(item.slip_path,600);if(data?.signedUrl)urls[item.id]=data.signedUrl}
    setSlipUrls(urls);setStatus('พร้อมตรวจสลิป')
  }
  useEffect(()=>{load()},[tenantId])
  const save=async(e:FormEvent)=>{e.preventDefault();setSaving(true);const {error}=await supabase.from('tenants').update({rental_price_monthly:base,room_price_monthly:perRoom,licensed_room_count:limit,updated_at:new Date().toISOString()}).eq('id',tenantId);setSaving(false);setStatus(error?error.message:'บันทึกราคาและสิทธิ์แล้ว');if(!error)await load()}
  const addRoomQuota=async()=>{const qty=Math.max(1,Math.floor(addRooms||1));const next=limit+qty;setSaving(true);setStatus('กำลังเพิ่มจำนวนห้อง...');const {error}=await supabase.from('tenants').update({licensed_room_count:next,updated_at:new Date().toISOString()}).eq('id',tenantId);setSaving(false);if(error){setStatus(error.message);return}setAddRooms(1);setStatus(`เพิ่มสิทธิ์แล้ว เป็น ${next} ห้อง`);await load()}
  const approve=async(r:Req)=>{if(!r.slip_path){setStatus('ยังไม่มีสลิปให้ตรวจ');return}setStatus('กำลังรับชำระและต่ออายุ...');const {error}=await supabase.rpc('platform_review_rental_request',{p_request_id:r.id,p_action:'approve',p_reason:null});setStatus(error?error.message:`รับชำระแล้ว · ต่อ ${r.renewal_months} เดือน · เปิด ${r.room_count} ห้อง`);await load()}
  const reject=async(r:Req)=>{const reason=window.prompt('เหตุผลที่ปฏิเสธสลิป เช่น ยอดไม่ตรง / รูปไม่ชัด / ตรวจไม่พบรายการ');if(!reason)return;setStatus('กำลังปฏิเสธสลิป...');const {error}=await supabase.rpc('platform_review_rental_request',{p_request_id:r.id,p_action:'reject',p_reason:reason});setStatus(error?error.message:'ปฏิเสธแล้ว รอ OA ส่งรายการใหม่');await load()}
